...
public class TableServiceImpl implements TableService {
    ...
    public List<ListQueueParamsDTO> getListQueueParams() {
        return tableDAO.getListQueueParams();
    }

    @Override
    public List<Map> getListQueueAccess() {
    ...
    @Override
    public List<List<String>> getSelectedFields(int userIndex, String tableName) {
        List<ListQueueParamsDTO> list = tableDAO.getListQueueParams();
        ...
        List<Map> searchField = tableDAO.getListQueueAccess(dynamicTable);

        for (Map map : searchField) {
            List<String> searchString = new ArrayList<>();
            for (ListQueueParamsDTO listQueueParamsDTO : list) {
                if (map.get(listQueueParamsDTO.getParam()).equals("Y")) {
                    searchString.add(listQueueParamsDTO.getParam());
                }
            }
            if (!CollectionUtils.isEmpty(searchString)) {
    ...
    @Override
    public List<Map> getTransactionType(DynamicTable dynamicQuery) {
        return tableDAO.getTransactionType(dynamicQuery);
    }
    ...
    @Override
    public List<Map> getBranchPendingTXN(DynamicTable dynamicQuery) {
        return tableDAO.getBranchPendingTXN(dynamicQuery);
    }

    @Override
    public List<Map> getSSCDraftedTXN(DynamicTable dynamicQuery) {
        return tableDAO.getSSCDraftedTXN(dynamicQuery);
    }

    @Override
    public List<Map> getBranchDraftedTXN(DynamicTable dynamicQuery) {
        return tableDAO.getBranchDraftedTXN(dynamicQuery);
    }

    @Override
    public List<Map> getSSCPendingTXN(DynamicTable dynamicQuery) {
        return tableDAO.getSSCPendingTXN(dynamicQuery);
    }

    @Override
    public List<Map> getAMLPendingTXN(DynamicTable dynamicQuery) {
        return tableDAO.getAMLPendingTXN(dynamicQuery);
    }
    ...
    @Override
    public List<Map> getTransactionID(DynamicTable dynamicQuery) {
        return tableDAO.getTransactionID(dynamicQuery);
    }
    ...
    public List<String> getExtTableDetails(String product) {
        return tableDAO.getExtTableDetails(product);
    }
    ...
    public List<CallbackAmendmentHistoryResponseDTO> callbackAmendmentHistory(String pid) {
        ...
        for (CallbackAmendmentHistoryDTO callbackAmendmentHistory : callbackAmendmentHistoryDTO) {
            ...
            callbackHistoryResponse.setDateTime(new SimpleDateFormat("dd-MM-yyyy HH:mm:ss").format(callbackAmendmentHistory.getActionDateTime()));
    ...
    public List<UtilizationHistoryResponseDTO> getUtilizationHistory(String pid) {
        ...
        for (UtilizationHistoryDTO utilizationHistory : utilizationHistoryDTO) {
            ...
            Map<String, Integer> currencyConfigMap = populateCurrency();

            String ccy1Amount = String.format("%." + currencyConfigMap.get(utilizationHistory.getCcy1()) +
                    "f", utilizationHistory.getCcy1UtilizationAmount()) +
                    " " + utilizationHistory.getCcy1();

            String ccy2Amount = String.format("%." + currencyConfigMap.get(utilizationHistory.getCcy2()) +
                    "f", utilizationHistory.getCcy1UtilizationAmount()) +
                    " " + utilizationHistory.getCcy2();

            utilizationHistoryResponse.setUserName(utilizationHistory.getActionUser());
            utilizationHistoryResponse.setDateTime(utilizationHistory.getActionDateTime());
            utilizationHistoryResponse.setAction(utilizationHistory.getAction());
            utilizationHistoryResponse.setCcy1Amount(ccy1Amount);
            utilizationHistoryResponse.setCcy2Amount(ccy2Amount);
            utilizationHistoryResponse.setDepartment(utilizationHistory.getDepartment());
    ...
    private String validateTransactionForCallbackAmendment(CallbackStatus callbackStatus, String pid) {
        ...
        if (callbackStatus == null || callbackStatus.getTransactionStatus() == null) {
            ...
            ngLogger.errorLog("Error in fetching transaction and utilization status for process instance Id" + pid);
                ...
                !(!StringUtils.hasText(callbackStatus.getUtilizationStatus()) || "UnUtilized".equalsIgnoreCase(callbackStatus.getUtilizationStatus()))) {

            errorMessage = "Transaction should not have any effective utilization";
        }
...
//        }
    ...
    private Map<String, Integer> populateCurrency() {
        Map<String, Integer> currencyConfigMap = new HashMap<>();
        List<Map<String, String>> currencyConfig = tableDAO.getCurrencyConfig();
